...
public class CameraPreView {
    ...
    CameraPreView(){
        mVertices = ByteBuffer.allocateDirect(QUARD_COOREDS.length * Float.SIZE / 8) // Float.SIZE / 8 == 4 , 4라고 해도 된다
                .order(ByteOrder.nativeOrder()).asFloatBuffer(); // --> Ex_02 에서 몇번에 걸쳐서 한거 한번에 구현한것
        mVertices.put(QUARD_COOREDS);
        mVertices.position(0);

        mTexCoords = ByteBuffer.allocateDirect(QUARD_TEXCOOREDS.length * Float.SIZE / 8)
                .order(ByteOrder.nativeOrder()).asFloatBuffer();
        mTexCoords.put(QUARD_TEXCOOREDS);
        mTexCoords.position(0);

        mTextCoordsTransformed = ByteBuffer.allocateDirect(QUARD_TEXCOOREDS.length * Float.SIZE / 8)
                .order(ByteOrder.nativeOrder()).asFloatBuffer();
    }
    ...
    void init(){
        ...
        // GLES20.glTexParameteri : 축소, 확대 필터을 설정, 점의 경계를 부드럽게 보느냐 반복시킬것인가 등등 설정
...
//        GLES20.glGetShaderiv(vShader, GLES20.GL_COMPILE_STATUS, compiled, 0); // 검사하기 위한 코드
